    ...
    <main id="contenido-principal">
      ...
      <section className="border-b border-(--color-border) bg-(--color-surface) py-12 lg:py-16">
        <div className="container">
          ...
          >
            <Link
              href="/productos"
              className="font-semibold hover:text-(--color-brand-red)"
            >
              Productos
            </Link>
            <span aria-hidden="true">/</span>
            <Link
              href={`/productos?categoria=${product.categoryId}`}
              className="font-semibold hover:text-(--color-brand-red)"
            >
          ...
          <div className="mt-8 grid gap-10 lg:grid-cols-2 lg:items-center">
            <div className="relative aspect-square overflow-hidden border border-(--color-border) bg-white">
              {product.image && (
                <Image
                  src={product.image}
                  alt={`${product.name}, equipo fabricado por Intraud`}
                  fill
                  priority
            ...
            <div>
              <p className="text-sm font-semibold tracking-[0.16em] text-(--color-brand-red) uppercase">
                {product.subcategoryName ?? product.categoryName}
              </p>
              <h1 className="font-display mt-4 text-5xl font-semibold leading-none tracking-tight text-balance uppercase sm:text-6xl lg:text-7xl">
                {product.name}
              </h1>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                ...
                {product.datasheetUrl && (
                  <a
                    ...
                    className="inline-flex min-h-12 items-center justify-center gap-2 rounded-sm border border-(--color-foreground) px-6 font-semibold transition-colors hover:bg-black hover:text-white"
                  >
                    <svg
                      aria-hidden="true"
                      viewBox="0 0 24 24"
              ...
              <p className="mt-5 text-sm text-(--color-muted-foreground)">
                Fabricación nacional · Asistencia técnica y repuestos
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 lg:py-20">
        <div className="container max-w-5xl">
          ...
          <h2 className="font-display text-4xl font-semibold tracking-tight uppercase sm:text-5xl">
            Especificaciones
          </h2>
          <dl className="mt-8 border-t border-(--color-border)">
            {product.specifications.map((specification) => (
              <div
                ...
                className="grid gap-2 border-b border-(--color-border) py-4 sm:grid-cols-2 sm:gap-8"
              >
                <dt className="font-semibold">{specification.label}</dt>
                <dd className="text-(--color-muted-foreground)">
                  {specification.value}
                </dd>
              </div>
            ))}
        ...
        <section className="border-t border-(--color-border) bg-(--color-surface) py-16 lg:py-20">
          <div className="container">
            <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
              ...
              <Link
                href={`/productos?categoria=${product.categoryId}`}
                className="inline-flex min-h-11 items-center font-semibold hover:text-(--color-brand-red)"
              >
                Ver toda la línea →
              </Link>
            </div>
